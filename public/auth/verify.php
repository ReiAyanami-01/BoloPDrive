...
<head>
  ...
  <title>Verificar email — BOLO P-Drive</title>
  <style>
    :root{ --rei-bg:#f6f8ff; --rei-line:#d7def0; --rei-primary:#6aa3ff; --radius:16px; }
    *{box-sizing:border-box} html,body{height:100%}
    ...
    .card{width:100%;max-width:480px;background:#fff;border:1px solid var(--rei-line);border-radius:var(--radius);
      box-shadow:0 12px 40px rgba(20,30,60,.12);padding:24px}
...
<div class="wrap">
  <div class="card">
    ...
    <form method="post" action="<?= BASE_URL ?>/../api/verify_email.php">
      <label>Email</label>
      <input type="email" name="email" value="<?= htmlspecialchars($email) ?>" required>

      <label>Código de verificación (6 dígitos)</label>
      <!-- cambio: name="code" (antes: name="otp") -->
      <input
        type="text"
        name="code"
        inputmode="numeric"
      ...
      <div class="links">
        <a href="<?= BASE_URL ?>/auth/register.php">Volver al registro</a>
        <a href="<?= BASE_URL ?>/auth/forgot.php">Olvidé mi contraseña</a>
      </div>
    </form>
  </div>
</div>
